...
/*PurpleHairZombie::PurpleHairZombie(QObject *parent)
    : ZombieBase{parent}
{}*/
int PurpleHairZombie::ChangePosToRect(QPointF point)
{
...
PurpleHairZombie::PurpleHairZombie(QGraphicsScene *&scene, QMap<int, QPair<QPointF, PlantBase*>> &plantMap,QMap<int, QPair<QPointF, ZombieBase*>> &zombieMap,int rect,QList<QRectF> gridRects,QMap<int,QPair<QPointF, QString>>& gridcenterMap)
    :ZombieBase(4000,1), scene(scene), plantMap(plantMap), zombieMap(zombieMap), currentRect(rect), gridRects(gridRects), gridcenterMap(gridcenterMap)

{

    zombieMovie = new QMovie(":/image/purple hair zombie_transparent.png");
    ...
    connect(moveTimer, &QTimer::timeout, this, &PurpleHairZombie::moveForward);
    moveTimer->start(1000);

    attackTimer = new QTimer(this);
    connect(attackTimer, &QTimer::timeout, this, &PurpleHairZombie::attackZombie);
}

void PurpleHairZombie::moveForward()
{
...
void PurpleHairZombie::attackZombie()
{
    if (this->getHealth() <= 0) {
