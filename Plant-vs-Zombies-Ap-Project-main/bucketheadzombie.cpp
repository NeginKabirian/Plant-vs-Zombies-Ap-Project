...
#include <boomerang.h>
#include<plummine.h>
int BucketHeadZombie::ChangePosToRect(QPointF point)
{
    ...
    : ZombieBase(1950,50), scene(scene), plantMap(plantMap), zombieMap(zombieMap), currentRect(rect), gridRects(gridRects), gridcenterMap(gridcenterMap)
{
    ...
    connect(moveTimer, &QTimer::timeout, this, &BucketHeadZombie::moveForward);
    moveTimer->start(2000);

    attackTimer = new QTimer(this);
    connect(attackTimer, &QTimer::timeout, this, &BucketHeadZombie::attackZombie);
}

void BucketHeadZombie::moveForward()
{
